...
class CoinDetailState {
  ...
  CoinDetailState._({required this.status, this.coinDetail, this.errorMessage});
...
class CoinDetailNotifier extends StateNotifier<CoinDetailState> {
  ...
  Future<void> fetchCoinDetail(String id) async {
    try {
      ...
      log('Error fetching coin detail data for $id', error: e, stackTrace: StackTrace.current);
